...
class ProximityRangingClusterRangingTriggerConditionStruct(
  val startTimeDelay: ULong,
  val endTimeDelay: Optional<ULong>,
  ...
  companion object {
    ...
    fun fromTlv(
      ...
      val startTimeDelay = tlvReader.getULong(ContextSpecificTag(TAG_START_TIME_DELAY))
      val endTimeDelay =
        if (tlvReader.isNextTag(ContextSpecificTag(TAG_END_TIME_DELAY))) {
          Optional.of(tlvReader.getULong(ContextSpecificTag(TAG_END_TIME_DELAY)))
        } else {
          Optional.empty()
        }
