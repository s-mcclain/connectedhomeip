...
class ProximityRangingClusterRangingMeasurementDataStruct(
  ...
  val timeOfMeasurement: UInt,
  val distance: Short,
  val accuracy: Optional<UShort>,
  ...
  companion object {
    ...
    fun fromTlv(
      ...
      val timeOfMeasurement = tlvReader.getUInt(ContextSpecificTag(TAG_TIME_OF_MEASUREMENT))
      val distance = tlvReader.getShort(ContextSpecificTag(TAG_DISTANCE))
      val accuracy =
        if (tlvReader.isNextTag(ContextSpecificTag(TAG_ACCURACY))) {
          Optional.of(tlvReader.getUShort(ContextSpecificTag(TAG_ACCURACY)))
        } else {
          Optional.empty()
        }
      val rdr =
