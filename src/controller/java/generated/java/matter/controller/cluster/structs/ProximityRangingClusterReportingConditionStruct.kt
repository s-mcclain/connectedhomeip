...
class ProximityRangingClusterReportingConditionStruct(
  val minDistanceCondition: Optional<Short>,
  val maxDistanceCondition: Optional<Short>,
  val accuracyCondition: Optional<Short>,
  val startTimePeriod: Optional<UInt>,
  val endTimePeriod: Optional<UInt>,
) {
  override fun toString(): String = buildString {
    ...
    append("\tstartTimePeriod : $startTimePeriod\n")
    append("\tendTimePeriod : $endTimePeriod\n")
    append("}\n")
  }
  ...
  fun toTlv(tlvTag: Tag, tlvWriter: TlvWriter) {
    tlvWriter.apply {
      ...
      if (startTimePeriod.isPresent) {
        val optstartTimePeriod = startTimePeriod.get()
        put(ContextSpecificTag(TAG_START_TIME_PERIOD), optstartTimePeriod)
      }
      if (endTimePeriod.isPresent) {
        val optendTimePeriod = endTimePeriod.get()
        put(ContextSpecificTag(TAG_END_TIME_PERIOD), optendTimePeriod)
      }
      endStructure()
    }
  }
  ...
  companion object {
    ...
    private const val TAG_START_TIME_PERIOD = 3
    private const val TAG_END_TIME_PERIOD = 4
    ...
    fun fromTlv(
      ...
      val minDistanceCondition =
        if (tlvReader.isNextTag(ContextSpecificTag(TAG_MIN_DISTANCE_CONDITION))) {
          Optional.of(tlvReader.getShort(ContextSpecificTag(TAG_MIN_DISTANCE_CONDITION)))
        } else {
          Optional.empty()
        }
      val maxDistanceCondition =
        if (tlvReader.isNextTag(ContextSpecificTag(TAG_MAX_DISTANCE_CONDITION))) {
          Optional.of(tlvReader.getShort(ContextSpecificTag(TAG_MAX_DISTANCE_CONDITION)))
        } else {
          Optional.empty()
        }
      val accuracyCondition =
        if (tlvReader.isNextTag(ContextSpecificTag(TAG_ACCURACY_CONDITION))) {
          Optional.of(tlvReader.getShort(ContextSpecificTag(TAG_ACCURACY_CONDITION)))
        } else {
          Optional.empty()
        }
      val startTimePeriod =
        if (tlvReader.isNextTag(ContextSpecificTag(TAG_START_TIME_PERIOD))) {
          Optional.of(tlvReader.getUInt(ContextSpecificTag(TAG_START_TIME_PERIOD)))
        } else {
          Optional.empty()
        }
      val endTimePeriod =
        if (tlvReader.isNextTag(ContextSpecificTag(TAG_END_TIME_PERIOD))) {
          Optional.of(tlvReader.getUInt(ContextSpecificTag(TAG_END_TIME_PERIOD)))
        } else {
          Optional.empty()
        }
