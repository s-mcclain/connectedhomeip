...
class ProximityRangingCluster(
  ...
  class StartRangingResponse(
    val sessionID: UByte,
    val targetWiFiDevIK: ByteArray?,
    val targetBLEDeviceId: ULong?,
    val resultCode: UByte,
  )
  ...
  suspend fun startRangingRequest(
    ...
    BTChannelSoundingDeviceRoleConfig:
      ProximityRangingClusterBTChannelSoundingDeviceRoleConfigStruct?,
    UWBRangingDeviceRoleConfig: ProximityRangingClusterUWBRangingDeviceRoleConfigStruct?,
    frequencyBand: UByte?,
    bandwidth: UByte?,
    securityMode: UByte?,
    ...
    val TAG_BT_CHANNEL_SOUNDING_DEVICE_ROLE_CONFIG_REQ: Int = 3
    BTChannelSoundingDeviceRoleConfig?.let {
      BTChannelSoundingDeviceRoleConfig.toTlv(
        ContextSpecificTag(TAG_BT_CHANNEL_SOUNDING_DEVICE_ROLE_CONFIG_REQ),
        tlvWriter,
      )
    }

    val TAG_UWB_RANGING_DEVICE_ROLE_CONFIG_REQ: Int = 4
    UWBRangingDeviceRoleConfig?.let {
      UWBRangingDeviceRoleConfig.toTlv(
        ContextSpecificTag(TAG_UWB_RANGING_DEVICE_ROLE_CONFIG_REQ),
        tlvWriter,
      )
    }

    val TAG_FREQUENCY_BAND_REQ: Int = 5
    frequencyBand?.let { tlvWriter.put(ContextSpecificTag(TAG_FREQUENCY_BAND_REQ), frequencyBand) }

    val TAG_BANDWIDTH_REQ: Int = 6
    bandwidth?.let { tlvWriter.put(ContextSpecificTag(TAG_BANDWIDTH_REQ), bandwidth) }

    val TAG_SECURITY_MODE_REQ: Int = 7
    securityMode?.let { tlvWriter.put(ContextSpecificTag(TAG_SECURITY_MODE_REQ), securityMode) }

    val TAG_TRIGGER_REQ: Int = 8
    trigger.toTlv(ContextSpecificTag(TAG_TRIGGER_REQ), tlvWriter)

    val TAG_REPORTING_CONDITION_REQ: Int = 9
    ...
    val TAG_TARGET_WI_FI_DEV_IK: Int = 1
    var targetWiFiDevIK_decoded: ByteArray? = null

    val TAG_TARGET_BLE_DEVICE_ID: Int = 2
    var targetBLEDeviceId_decoded: ULong? = null

    val TAG_RESULT_CODE: Int = 3
    ...
    while (!tlvReader.isEndOfContainer()) {
      ...
      } else if (tag == ContextSpecificTag(TAG_TARGET_WI_FI_DEV_IK)) {
        targetWiFiDevIK_decoded =
          if (tlvReader.isNull()) {
      ...
      } else if (tag == ContextSpecificTag(TAG_TARGET_BLE_DEVICE_ID)) {
        targetBLEDeviceId_decoded =
          if (tlvReader.isNull()) {
            tlvReader.getNull(tag)
            null
          } else {
      ...
      } else if (tag == ContextSpecificTag(TAG_RESULT_CODE)) {
